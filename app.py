# app.py — عربي سايكو: AI + DSM5 إرشادي + CBT + اختبارات نفسية وشخصية + تحويل طبي
# Python 3.10+ | python-telegram-bot v21.x
...
if not BOT_TOKEN:
    raise RuntimeError("يرجى ضبط TELEGRAM_BOT_TOKEN (من @BotFather)")

# مفاتيح الذكاء الاصطناعي (OpenAI/OpenRouter متوافقة مع واجهة chat/completions)
AI_BASE_URL = (os.getenv("AI_BASE_URL") or "").strip()   # مثال OpenAI: https://api.openai.com/v1
...
# Webhook على Render/غيره
PUBLIC_URL = (os.getenv("PUBLIC_URL")
              or os.getenv("WEBHOOK_URL")
              or os.getenv("RENDER_EXTERNAL_URL")
              or "").strip()
WEBHOOK_SECRET = (os.getenv("WEBHOOK_SECRET") or "").strip()  # إن أردت مسارًا ثابتًا
...
PERSONALITY_KB = ReplyKeyboardMarkup(
    [
        ["TIPI الخمسة الكبار", "SAPAS اضطراب شخصية"],
        ["MSI-BPD حدّية"],
        ["◀️ رجوع"]
    ],
    resize_keyboard=True
...
def ai_call(user_content: str, history: List[Dict[str,str]], dsm_mode: bool) -> str:
    if not (AI_BASE_URL and AI_API_KEY and AI_MODEL):
        return "تعذّر الاتصال بالذكاء الاصطناعي حاليًا (تحقّق من المفتاح/العنوان/النموذج)."
    ...
    try:
        r = requests.post(f"{AI_BASE_URL.rstrip('/')}/chat/completions",
...
TIPI = Survey("tipi","TIPI — الخمسة الكبار (10 بنود)",
...
def referral_keyboard():
    ...
    if not rows:
        rows.append([InlineKeyboardButton("أرسل رابط/رقم التواصل", url="https://t.me/")])
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.effective_chat.send_message(
        ...
        "مساعد نفسي عربي (تعليمي/سلوكي)، ولستُ بديلاً عن التشخيص الطبي.\n"
        "اختر من الأزرار بالأسفل أو اكتب مشكلتك لبدء جلسة.",
        reply_markup=TOP_KB
    )
    return MENU
...
async def top_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if t in ("القائمة ◀️", "القائمة", "رجوع", "◀️"):
        await update.message.reply_text("القائمة:", reply_markup=TOP_KB)
        return MENU
    ...
    if "عربي سايكو" in t:
        ...
        await update.message.reply_text(
            "أنا **عربي سايكو** 🤖 مساعد نفسي مدعوم بالذكاء الاصطناعي "
            "(دعم/تعليم سلوكي، وليس تشخيصًا طبيًا).",
            reply_markup=kb
        )
        return MENU
    ...
    if any(k in t for k in ("اختبارات الشخصية", "اختبار الشخصية", "الشخصية")):
        await update.message.reply_text("🧩 اختبارات الشخصية — اختر اختبارًا:", reply_markup=PERSONALITY_KB)
        return TESTS_MENU

    if "اضطرابات الشخصية" in t:
        await send_long(update.effective_chat, PD_TEXT, TOP_KB)
        return MENU
...
async def dsm_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.chat.send_message(
        ...
        "صف الأعراض (المدة/الشدة/التأثير)، وسأقترح مسارات تقييم وتمارين CBT مناسبة.",
...
async def tests_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    key = {
        ...
        "TIPI الخمسة الكبار":"tipi","TIPI الخمسة الكبار (10 بنود)":"tipi",
        "SAPAS اضطراب شخصية":"sapas","MSI-BPD حدّية":"msi",
        "ISI-7 أرق":"isi7","PSS-10 ضغوط":"pss10","WHO-5 رفاه":"who5","K10 ضيق نفسي":"k10"
    }.get(t)

    if key is None:
        # لو كنت داخل قائمة الشخصية فرجع نفس اللوحة
        if any(k in t for k in ("اختبارات الشخصية", "اختبار الشخصية")):
            await update.message.reply_text("🧩 اختبارات الشخصية — اختر اختبارًا:", reply_markup=PERSONALITY_KB)
            return TESTS_MENU
    ...
    s_map = {"phq9":PHQ9, "gad7":GAD7, "minispin":MINISPIN, "tipi":TIPI,
...
async def survey_flow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "bin" in context.user_data:
        ...
        await update.message.reply_text(msg, reply_markup=TESTS_KB)
...
def main():
    ...
    if PUBLIC_URL:
        url_path = WEBHOOK_SECRET if WEBHOOK_SECRET else BOT_TOKEN
        app.run_webhook(
            listen="0.0.0.0",
            port=PORT,
            url_path=url_path,
