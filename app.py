...
CBT_INTRO = (
    ...
    "نهج عملي يركّز على العلاقة بين <b>الأفكار ↔ المشاعر ↔ السلوك</b>.\n"
    "الفكرة: لاحِظ الفكرة غير المفيدة واستبدِلها بمتوازنة، وجرّب سلوكيات صغيرة بنظام، فيتحسّن المزاج والقلق تدريجيًا.\n"
    "ابدأ ببطاقة واحدة اليوم وطبّق خطواتها 5–10 دقائق."
)
...
CBT_CARDS = [
    ...
    (
        "الأسئلة العشرة",
        "الدليل؟ البدائل؟ أسوأ/أفضل/أرجح سيناريو؟ لو صديق مكاني ماذا سأقول له؟\n"
        "هل أتجاهل جوانب إيجابية؟ هل أعتمد على شعور لا على حقائق؟"
    ),
    (
        "التعرّض التدريجي (للقلق/الهلع)",
        "اكتب سلّم مواقف من 0–10.\n"
        "ابدأ من 3–4/10: تعرّض آمن بلا سلوكيات أمان (طمأنة/هروب) حتى ينخفض القلق، ثم اصعد درجة."
    ),
    (
        "التنشيط السلوكي (للمزاج)",
        "نشاطان صغيران يوميًا: ١) ممتع ٢) نافع.\n"
        "قاعدة 5 دقائق للبدء. سجّل المزاج قبل/بعد ولاحِظ التحسّن التراكمي."
    ),
    (
        "الاسترخاء والتنفس",
        "تنفّس 4–7–8 ×6 مرات.\n"
        "شد/إرخِ العضلات من القدم للرأس: شدّ 5 ثوانٍ ثم إرخِ 10 ثوانٍ."
    ),
    (
        "اليقظة الذهنية",
        "تمرين 5–4–3–2–1 للحواس: 5 ترى، 4 تلمس، 3 تسمع، 2 تشمّ، 1 تتذوّق.\n"
        "ارجع للحاضر بدون حكم."
    ),
    (
        "حلّ المشكلات",
        "عرّف المشكلة بدقّة → بدائل صغيرة → خطوة SMART → جرّب → قيّم وعدّل."
    ),
    (
        "بروتوكول النوم",
...
def cbt_menu(chat_id):
    # NOTE: إن لم تكن لديك دالة menu_kb() استبدلها بـ reply_kb()
    send(chat_id, CBT_INTRO, menu_kb() if 'menu_kb' in globals() else reply_kb())
    rows = [[{"text": title, "callback_data": "cbt:" + title}] for (title, _) in CBT_CARDS]
    send(chat_id, "اختر بطاقة من العلاج السلوكي:", inline(rows))

def cbt_send(chat_id, title):
    for (t, body) in CBT_CARDS:
        if t == title:
            send(chat_id, f"<b>{t}</b>\n{body}", menu_kb() if 'menu_kb' in globals() else reply_kb())
            break
